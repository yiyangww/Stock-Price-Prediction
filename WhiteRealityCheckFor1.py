...
def bootstrap(ser):
    ...
    # Comment out this line since we're already using log returns
    # ser = np.log(ser+1)
...
def test_all_models():
    """
    Test all models using White Reality Check
    """
    models = ['lr', 'ridge', 'svr', 'xgb']
    
    print("=" * 60)
    print("WHITE REALITY CHECK FOR ALL MODELS")
    print("=" * 60)
    
    for model in models:
        try:
            filename = f'returns_{model}_logreturns.csv'
            print(f"\n{'='*20} {model.upper()} {'='*20}")
            ...
            bootstrap(returns.iloc[:, 0])  # Use first column
            
            print(f"{'='*50}")
            
        except FileNotFoundError:
            print(f"File {filename} not found. Skipping {model.upper()}.")
